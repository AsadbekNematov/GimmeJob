...
function Question() {
  ...
  const handleEnoughForToday = () => {
    ...
    setFinalScore(parseInt(messages[messages.length-1].content))
    const data = {
      profession: newJobPosition,
      score: finalScore,
      date: 'right now'
    };
    db.collection('sessions').doc('newest interview practice').set(data);
  };
